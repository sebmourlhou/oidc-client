...
has 'private_key_file' => (
  ...
  isa     => 'Maybe[Str]',
  lazy    => 1,
  default => sub { shift->config->{private_key_file} },
);
...
has 'private_jwk' => (
  ...
  isa     => 'Maybe[HashRef]',
  lazy    => 1,
  default => sub { shift->config->{private_jwk} },
);

has 'private_jwk_file' => (
  is      => 'ro',
  isa     => 'Maybe[Str]',
  lazy    => 1,
  default => sub { shift->config->{private_jwk_file} },
);
...
has 'client_assertion_audience' => (
  is      => 'ro',
  isa     => 'Maybe[Str]',
  lazy    => 1,
  default => sub { shift->config->{client_assertion_audience} },
);
...
sub get_token {
  ...
  if ($grant_type eq 'authorization_code') {
    ...
    foreach my $required_field (qw/username password/) {
      $form->{$required_field} = ($params{$required_field} || $self->config->{$required_field})
