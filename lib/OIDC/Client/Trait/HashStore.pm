...
after 'install_accessors' => sub {
  ...
  my $name = $self->name;
  $realclass->add_method("read_${name}"   => sub { return reach_data( $_[0]->$name, $_[1]) });
  $realclass->add_method("write_${name}"  => sub { return affect_data($_[0]->$name, $_[1], $_[2]) });
  $realclass->add_method("delete_${name}" => sub { return delete_data ($_[0]->$name, $_[1]) });
};
