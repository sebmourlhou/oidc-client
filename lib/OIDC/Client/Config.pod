...
in the C<Authorization> header for each request (Authorization Server).
...
an authentication attempt or if an error occurs when the token is retrieved
in exchange for the code.

If this path is not configured, an error, with the message configured
in C<authentication_error_message> or else a generic message, is thrown with
an HTTP 401 code.

=head2 authentication_error_message

Message used to return an error if C<authentication_error_path> is not set.
...
Options to be transferred to the library
[Crypt::JWT](https://metacpan.org/pod/Crypt::JWT#decode_jwt) used to validate
and decode a JWT token.
...
The C<exp> claim must be present
...
=head2 provider."provider".jwt_claim_key

Used to set the names of the attributes (claims) of the JWT token for creating
the C<identity> hashref.
